...
function initApp() {
  ...
  // ALWAYS try to load products on index page - fetchProducts handles fallback
...
// === FALLBACK DATA ===
const FALLBACK_PRODUCTS = [{"id":4,"name":"🕰️Aura Milano Classic","price":649.00,"description":"Aura Milano Classic: Everyday Elegance, Redefined.\n\nSimplicity meets premium class in the Aura Milano Classic. With its deep black dial and clean design, this is the perfect accessory for anyone who appreciates timeless, bold style without the fuss.\n\n✅ Key Features:\n⚫ Matte Black Dial – Sleek and versatile, perfectly complements any outfit, from traditional panjabi to formal suits.\n⌚ Clean Markers – No distractions, just pure minimalist style for a sophisticated look.\n🧭 Advanced Japanese Movement – Powered by a reliable dual-battery system for years of precise timekeeping without frequent changes.\n💦 Splash Resistant – Don’t worry about rain or accidental splashes.\n🔗 Premium Adjustable Strap – A comfortable stainless steel bracelet, easily resized at home with the FREE chain cutter included in the box.\n\n📦 What’s in the Box?\n1x Aura Milano Classic Watch\n1x Free Premium Chain Cutter (for perfect fit!)\n1x Luxury Gift Box\n\n🧑‍💼 Perfect For:\n• Daily office wear\n• Formal events and weddings\n• A sophisticated gift for him\n• Students & young professionals\n🛒 Order Your Aura Milano Classic Today!\n🚚 Fast Delivery: All over Bangladesh in just 2-4 days!\n💳 Easy Payment: Cash on Delivery (COD)\n📞 Have questions? Message us for instant support!","short_description":"🕰️Aura Milano Classic\n\nSimplicity meets class in the Aura Milano Classic — a watch designed for everyday elegance. With its deep black dial and minimal hour markers, this is the perfect accessory for anyone who appreciates clean, bold design.","delivery_details":"Inside Dhaka 60tk-Outside Dhaka 120tk","image":"https://i.ibb.co.com/JWSZKDZB/10071.jpg","extra_images":[],"category":"everyday","badge":"Best Seller","is_featured":false,"created_at":"2025-11-25T06:28:17.551571+00:00","updated_at":"2025-11-25T06:28:17.551571+00:00","image2":"https://i.ibb.co.com/Rp9yxbZg/10073.jpg","image3":"https://i.ibb.co.com/k6Mc1kXZ/10069.jpg","image4":null},
{"id":1,"name":"Royal Sovereign","price":125000,"sale_price":95000,"is_on_sale":true,"description":"A masterpiece of engineering and design.","image":"assets/images/watch-1.png","category":"masterpiece","stock_quantity":3},
{"id":2,"name":"Elite Chronograph","price":45000,"description":"Precision timing for the modern professional.","image":"assets/images/watch-2.png","category":"elite","stock_quantity":10},
{"id":3,"name":"Midnight Phantom","price":28000,"description":"Sleek, dark, and mysterious.","image":"assets/images/watch-3.png","category":"exclusive","stock_quantity":0}];
...
async function fetchProducts() {
  ...
  if (!sb) {
    useFallback('No Supabase Client');
    return;
  }
  ...
  try {
    ...
    const fetchPromise = sb.from('products').select('*').order('created_at', { ascending: false });
    ...
    if (!products || products.length === 0) {
       console.warn('Supabase returned empty list');
...
function renderProducts(products, container) {
  container.innerHTML = products
    .map((product, index) => {
      ...
      const stockStatus = window.enhancedFeatures?.getStockStatus(
      ...
      return `
    <div class="product-card animate-on-scroll" style="animation-delay: ${
      index * 0.1
    }s; display: block; opacity: 1;" data-category="${
        product.category
      }" data-name="${product.name.toLowerCase()}" data-sku="${(
        product.sku || ""
      ).toLowerCase()}">
      <div class="product-image-wrapper">
        ...
        <div class="card-actions" style="justify-content: center; width: 100%; gap: 10px;">
          <a href="product-details.html?id=${
            product.id
          }" class="card-action-btn view-details-btn">
            VIEW DETAILS
          </a>
          <button class="card-action-btn wishlist-btn" data-id="${
            product.id
          }" onclick="window.toggleWishlist(${
        product.id
      }, this)" aria-label="Add to Wishlist">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
          </button>
        </div>
      </div>
      <div class="product-info">
        <h3 class="product-name"><a href="product-details.html?id=${
          product.id
        }">${product.name}</a></h3>
...
function renderCart() {
  ...
  if (cartSummaryContainer) {
    ...
    const totalElement =
      cartSummaryContainer.querySelector(".total .gold-text");
    const subtotalElement =
      cartSummaryContainer.querySelectorAll(".summary-row span")[1];
    const shippingElement = cartSummaryContainer
...
async function placeOrder() {
  ...
  try {
    ...
    localStorage.removeItem("cart");
    setTimeout(() => {
      window.location.href = `order-confirmation.html?id=ORD-${Date.now()
        .toString()
        .slice(-6)}`;
    }, 1500);
  }
...
function initBurgerMenu() {
  ...
  burgerMenu.addEventListener("click", (e) => {
    ...
    body.style.overflow = navBottom.classList.contains("active")
      ? "hidden"
      : "";
  });
...
function initHeroParallax() {
  ...
  heroSection.addEventListener("mousemove", (e) => {
    ...
    heroImage.style.transform = `perspective(1000px) rotateX(${
      -y * 10
    }deg) rotateY(${x * 10}deg) scale(1.05)`;
    particles.forEach((p, i) => {
      p.style.transform = `translate(${x * (i + 1) * 15}px, ${
        y * (i + 1) * 15
      }px)`;
    });
  });
...
function initScrollAnimations() {
  ...
  document
    .querySelectorAll(".animate-on-scroll, .section-divider")
...
function renderSearchPreview(query) {
  ...
  const previewHTML = matches
    .map((card) => {
      ...
      const link =
        card.querySelector("a.product-link-overlay")?.getAttribute("href") ||
        "#";
...
function initSearchBar() {
  ...
  searchInputs.forEach((searchInput) => {
    ...
    searchInput.addEventListener("input", (e) => {
      ...
      // Filter locally
      const matches = searchableProducts
        .filter(
          (p) =>
    ...
    const button =
      searchInput.parentElement.querySelector(".search-btn-modern");
    if (button) {
...
function renderSearchSuggestions(products, container) {
  if (!products || products.length === 0) {
    container.innerHTML =
      '<div class="preview-no-results">No products found</div>';
  ...
  const html = products
    .map((product) => {
      ...
      <a href="product-details.html?id=${product.id}" class="preview-item">
        <img src="${product.image}" alt="${
        product.name
      }" class="preview-image" onerror="this.src='assets/images/watch-1.png'">
...
async function checkWishlistStatus() {
  ...
  try {
    const { data: wishlistItems, error } = await getSupabaseClient()
      .from("wishlist")
      .select("product_id")
...
async function updateMobileNav() {
  ...
  if (mobileAccountBtn && session?.user) {
    ...
    mobileAccountBtn.querySelector("span").textContent = isAdmin
      ? "Admin"
      : "Profile";
  }
}
